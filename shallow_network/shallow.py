...
def dynamic_formula(n1, n2, k1, k2, r, a_init, num_time_steps, num_svds):
  ...
  sv_ratio = s*d_inv
  sv_traj = (s*d_inv)*(1-np.exp(-taus/d_inv)) + a_init*np.exp(-taus/d_inv)
  sv_trajectory_plots = np.zeros(sv_traj.shape)
  num_sig_svs = sv_ratio[sv_ratio > 0].shape[0]
  sv_trajectory_plots = sv_traj[:,:num_sig_svs]
...
if __name__ == "__main__":
  ...
  def updater(params, batch):
    ...
    Ws =params[0]
    return [params[0] + ((1/tau) * (sigma_yx - np.dot(Ws, sigma_xx)))]
  ...
  num_hidden = 50.0
  layer_sizes = [n1 + k1*2**n1, n2 + k2*2**n1]
  step_size = 0.005
  num_epochs = 300
  ...
  # Init params, plot initial loss and do SVD on the yx-covar matrix for the initial weights
  ...
  # Run generations of IL. We track the SVs of the yx-covar matrix from the network at the end of each epoch
  for epoch in range(num_epochs):
      ...
      # Get input-output mapping of network and its singular values
      full_map = params[0]
  ...
  plt.grid()
  plt.savefig('sh_quad_norms_plot.pdf')
  plt.close() 
